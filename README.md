...
* while 1 
    check the flag for silver 
    if silver search for silver token and avoid golden token 
    alignment of the robot in the direction of token 
    if no token then turn 
    if dist < thresholds  then grab token 
    rotate the robot and release the box 
